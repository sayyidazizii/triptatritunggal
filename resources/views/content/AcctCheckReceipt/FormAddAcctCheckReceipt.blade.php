...
<script>
	...
	$(document).ready(function(){
        ...
        if(!elements['check_receipt_due_date']){
            $("#check_receipt_due_date").val('');
        }
    ...
    function function_elements_add(name, value){
        $.ajax({
            type: "POST",
            url : "{{route('elements-add-check-receipt')}}",
            dataType: "html",
            data: {
                'name'      : name,
                'value'	    : value,
...
<nav aria-label="breadcrumb">
    <ol class="breadcrumb">
        ...
        <li class="breadcrumb-item active" aria-current="page">Tambah Penerimaan Giro</li>
    </ol>
</nav>
...
<form method="post" action="{{route('process-add-check-receipt')}}" enctype="multipart/form-data">
    @csrf
<h3 class="page-title">
    <b>Form Tambah Penerimaan Giro</b>
</h3>
<br/>
@if(session('msg'))
        ...
        <div class="card-body">
            <div class="row form-group">
                ...
                <div class="col-md-6">
                    ...
                    <input type ="date" class="form-control form-control-inline input-medium date-picker input-date" data-date-format="dd-mm-yyyy" type="text" name="check_receipt_date" id="check_receipt_date" onChange="changeDate(this.name, this.value);" value="{{$acctreceiptelements == null ? '' : $acctreceiptelements['check_receipt_date']}}" />
                </div>
            </div>
            <div class="row form-group">
                <div class="col-md-6">
                    <a class="text-dark">Nomor Giro<a class='red'> *</a></a>
                    <input class="form-control input-bb" type="text" name="check_number" id="check_number" onChange="function_elements_add(this.name, this.value)" value="{{$acctreceiptelements == null ? '' : $acctreceiptelements['check_number']}}"/>
                </div>
                <div class="col-md-6">
                    <a class="text-dark">Jatuh tempo<a class='red'> *</a></a>
                    <input type ="date" class="form-control form-control-inline input-medium date-picker input-date" data-date-format="dd-mm-yyyy" type="text" name="check_receipt_due_date" id="check_receipt_due_date" onChange="changeDate2(this.name, this.value);" value="{{$acctreceiptelements == null ? '' : $acctreceiptelements['check_receipt_due_date']}}" />
                </div>
            </div>
            <div class="row form-group">
                <div class="col-md-6">
            ...
            <div class = "row form-group">
                <div class="col-md-12 ">
                    ...
                    <div class="">
                        <textarea rows="3" type="text" class="form-control input-bb" name="check_receipt_description" onChange="function_elements_add(this.name, this.value);" id="check_receipt_description" >{{$acctreceiptelements == null ? '' : $acctreceiptelements['check_receipt_description']}}</textarea>
                    </div>
                </div>
            </div>
            <br/>
            ...
            <div class="row form-group">
                ...
                <div class="col-md-6">
                    <div class="form-group">
                        <a class="text-dark">Judul</a>
                        <input class="form-control input-bb" type="text" name="check_receipt_item_title" id="check_receipt_item_title" value=""/>
                    </div>
                </div>
            </div>
            ...
            <div class = "row form-group">
                <div class="col-md-6">
                    <div class="form-group">
                        ...
                        <input class="form-control input-bb" type="text" name="check_receipt_item_amount" id="check_receipt_item_amount" value=""/>
                    </div>
                </div>
            </div>
        </div>
                                ...
                                foreach ($acctreceiptitem AS $key => $val){
                                    echo"
                                        <tr>
                                            ...
                                            <td style='text-align  : left !important;'>".$AcctCheckReceipt->getAccountName($val['account_id_item'])."</td>
                                        ...
                                        ?>
                                            <td style='text-align  : center'>
                                                <a href="{{route('delete-check-receipt-array', ['record_id' => $key])}}" name='Reset' class='btn btn-danger btn-sm' onClick='javascript:return confirm(\"apakah yakin ingin dihapus ?\")'></i> Hapus</a>
                                            </td>
                                            <?php
                                            echo"
                                        </tr>
