...
<nav aria-label="breadcrumb">
    <ol class="breadcrumb">
        ...
        <li class="breadcrumb-item active" aria-current="page">Daftar Penerimaan Giro</li>
    </ol>
</nav>
...
<h3 class="page-title">
    <b>Daftar Penerimaan Giro</b> <small>Mengelola Penerimaan Giro </small>
</h3>
<br/>
<div id="accordion">
    <form  method="post" action="{{route('filter-check-receipt')}}" enctype="multipart/form-data">
    @csrf
...
<div class="card border border-dark">
    <div class="card-header bg-dark clearfix">
        ...
        <div class="form-actions float-right">
            <a   href="{{route('add-check-receipt')}}" name="Find" class="btn btn-sm btn-info" title="Add Data"><i class="fa fa-plus"></i> Tambah Penerimaan Giro Baru</a>
        </div>
    </div>
            ...
            <table id="example" style="width:100%" class="table table-striped table-bordered table-hover table-full-width">
                ...
                <tbody>
                    ...
                    <tr>
                        ...
                        <td>{{$AcctCheckReceipt->getCustomerName($val['customer_id'])}}</td>
                        <td>{{date('d/m/Y', strtotime($val['check_receipt_date']))}}</td>
                        <td>{{$val['check_receipt_no']}}</td>
                        <td>{{date('d/m/Y', strtotime($val['check_receipt_due_date']))}}</td>
                        <td>{{$val['check_number']}}</td>
                        <td>{{$val['check_receipt_title']}}</td>
                        <td style='text-align:right'>{{number_format($val['check_receipt_amount_total'], 0, '', '.')}}</td>
                        <td class="" style='text-align:center'>
                            <a type="button" class="btn btn-outline-warning btn-sm" href="{{route('detail-check-receipt', ['check_receipt_id' => $val['check_receipt_id']])}}" >Detail</a>
                            <a type="button" class="btn btn-outline-danger btn-sm"  href="{{route('void-check-receipt', ['check_receipt_id' => $val['check_receipt_id']])}}">Batal</a>
                        </td>
                    </tr>
                    <?php $no++?>
                    @endforeach
