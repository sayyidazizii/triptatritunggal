...
<nav aria-label="breadcrumb">
    <ol class="breadcrumb">
        ...
        <li class="breadcrumb-item"><a href="{{ url('sales-delivery-note/add/'.$sales_delivery_order_id) }}">Tambah Sales Delivery Note</a></li>
        <li class="breadcrumb-item active" aria-current="page">Detail Item Stock</li>
    </ol>
</nav>
    ...
    <div class="card border border-dark">
        <div class="card-header border-dark bg-dark">
            ...
            <div class="float-right">
                ...
                <button onclick="location.href='{{ url('sales-delivery-note/add/'.$sales_delivery_order_id) }}'" name="Find" class="btn btn-sm btn-info" title="Back"><i class="fa fa-angle-left"></i>  Kembali</button>
            </div>
        </div>
                                    ...
                                    @endphp
                                        @foreach ($detail_stock_sdo AS $key => $val)
                                                <tr>
                                                    ...
                                                    <td style='text-align  : left !important;'>{{$SalesDeliveryNote->getSalesOrderItemTypeName($val['sales_order_item_id'])}}</td>
                                                    <input type="hidden" name="sales_order_item_id_{{$no}}" id="sales_order_item_id_{{$no}}" value="{{$val['sales_order_item_id']}}">
                                                    <td style='text-align  : left !important;'>{{$SalesDeliveryNote->getSelectInvItemStock2($val['item_stock_id'])}}</td>
