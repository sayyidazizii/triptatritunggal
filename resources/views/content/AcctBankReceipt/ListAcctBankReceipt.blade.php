...
<nav aria-label="breadcrumb">
    <ol class="breadcrumb">
        ...
        <li class="breadcrumb-item active" aria-current="page">Daftar Penerimaan Bank</li>
    </ol>
</nav>
...
<h3 class="page-title">
    <b>Daftar Penerimaan Bank</b> <small>Mengelola Penerimaan Bank </small>
</h3>
<br/>
<div id="accordion">
    <form  method="post" action="{{route('filter-bank-receipt')}}" enctype="multipart/form-data">
    @csrf
...
<div class="card border border-dark">
    <div class="card-header bg-dark clearfix">
        ...
        <div class="form-actions float-right">
            <a   href="{{route('add-bank-receipt')}}" name="Find" class="btn btn-sm btn-info" title="Add Data"><i class="fa fa-plus"></i> Tambah Penerimaan Bank Baru</a>
        </div>
    </div>
            ...
            <table id="example" style="width:100%" class="table table-striped table-bordered table-hover table-full-width">
                ...
                <tbody>
                    ...
                    <tr>
                        ...
                        <td>{{$AcctBankReceipt->getCustomerName($val['customer_id'])}}</td>
                        <td>{{date('d/m/Y', strtotime($val['bank_receipt_date']))}}</td>
                        <td>{{$val['bank_receipt_no']}}</td>
                        <td>{{$val['bank_receipt_title']}}</td>
                        <td style='text-align:right'>{{number_format($val['bank_receipt_amount_total'], 0, '', '.')}}</td>
                        <td class="" style='text-align:center'>
                            <a type="button" class="btn btn-outline-warning btn-sm" href="{{route('detail-bank-receipt', ['bank_receipt_id' => $val['bank_receipt_id']])}}" >Detail</a>
                            <a type="button" class="btn btn-outline-danger btn-sm"  href="{{route('void-bank-receipt', ['bank_receipt_id' => $val['bank_receipt_id']])}}">Batal</a>
                        </td>
                    </tr>
                    <?php $no++?>
                    @endforeach
