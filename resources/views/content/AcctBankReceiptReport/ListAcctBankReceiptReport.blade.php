@inject('AcctBankReceiptReport', 'App\Http\Controllers\AcctBankReceiptReportController')
...
<nav aria-label="breadcrumb">
    <ol class="breadcrumb">
        ...
        <li class="breadcrumb-item active" aria-current="page">Laporan Penerimaan Bank</li>
    </ol>
</nav>
...
<h3 class="page-title">
    <b>Laporan Penerimaan Bank</b> <small>Daftar Penerimaan Bank </small>
</h3>
<br/>
<div id="accordion">
    <form  method="post" action="{{route('report-filter-bank-receipt')}}" enctype="multipart/form-data">
    @csrf
    ...
    <div class="card-body">
        <div class="table-responsive">
            <table id="example" style="width:100%" class="table table-striped table-bordered table-hover table-full-width">
                ...
                <tbody>
                    <?php $no =1; ?>
                    @foreach($acctreceipt as $receipt)
                    <tr>
                        <td style='text-align:center'>{{$no}}</td>
                        <td>{{$receipt['bank_receipt_date']}}</td>
                        <td>{{$receipt['bank_receipt_no']}}</td>
                        <td>{{$receipt['bank_receipt_title']}}</td>
                        <td style='text-align:right'>{{number_format($receipt['bank_receipt_amount_total'], 0, '', '.')}}</td>
                    </tr>
                    <?php $no++?>
                    @endforeach
                </tbody>
