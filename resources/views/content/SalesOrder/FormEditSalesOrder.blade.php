    ...
    <form method="post" action="{{route('process-add-sales-order')}}" enctype="multipart/form-data">
        ...
        <div class="card-body">
            <div class="row form-group">
                <div class="col-md-6">
                    ...
                    <input type ="date" class="form-control form-control-inline input-medium date-picker input-date" data-date-format="dd-mm-yyyy" type="text" name="sales_order_date" id="sales_order_date" onChange="function_elements_add(this.name, this.value);" value="{{$salesorder['sales_order_date']}}" style="width: 15rem;"/>
                </div>
                <div class="col-md-6">
                    ...
                    <input type ="date" class="form-control form-control-inline input-medium date-picker input-date" data-date-format="dd-mm-yyyy" type="text" name="sales_order_shipment_date" id="sales_order_shipment_date" onChange="function_elements_add(this.name, this.value);" value="{{$salesorder['sales_order_shipment_date']}}" style="width: 15rem;"/>
                </div>
            </div>
            <div class="row form-group">
                <div class="col-md-6">
                    <a class="text-dark">Gudang<a class='red'> *</a></a>
                    {!! Form::select('warehouse_id',  $warehouse, $salesorder['warehouse_id'], ['class' => 'selection-search-clear select-form', 'id' => 'warehouse_id']) !!}
                </div>
                <div class="col-md-6">
                    <a class="text-dark">Pemasok<a class='red'> *</a></a>
                    {!! Form::select('supplier_id',  $supplier, $salesorder['supplier_id'], ['class' => 'selection-search-clear select-form', 'id' => 'supplier_id']) !!}
                </div>
            </div>
            <div class="row form-group">
                <div class="col-md-12 ">
                    ...
                    <div class="">
                        <textarea rows="3" type="text" class="form-control input-bb" name="sales_order_remark" onChange="function_elements_add(this.name, this.value);" id="sales_order_remark" >{{$salesorder['sales_order_remark']}}</textarea>
                    </div>
                </div>
            </div>
        ...
        <div class="card-footer text-muted">
            <div class="form-actions float-right">
                <a type="submit" name="Save" class="btn btn-primary" title="Save" onclick="processAddArraySalesOrderItem()">Tambah</a>
            </div>
        </div>
    </div>
    </div>
                                    ...
                                    foreach ($salesorderitem AS $key => $val){
                                        echo"
                                            <tr>
                                                ...
                                                <td style='text-align  : left !important;'>".$SalesOrder->getItemCategoryName($val['item_category_id'])."</td>
                                                ...
                                                <?php
                                                    if($val['sales_order_item_id']){
                                                        echo"
                                                        <input class='form-control input-bb' type='text' name='total_price_all' id='total_price_all' value='".$val['sales_order_item_id']."'/>";
                                                    }"
                                                </td>";
                                                echo"
                                            </tr>
