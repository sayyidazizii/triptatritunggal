...
<script>
    ...
    function elements_add(name, value){
        $.ajax({
            type: "POST",
            url : "{{route('elements-add-warehouse-out-requisition')}}",
            dataType: "html",
            data: {
                'name'      : name,
                'value'	    : value,
	...
	$(document).ready(function(){
        ...
        var elements = {!! json_encode($warehouseoutelements) !!};
        ...
        if(!elements['warehouse_out_type_id']){
            $("#warehouse_out_type_id").select2("val", "0");
        }
        ...
        $("#item_stock_id").change(function(){
            ...
            $.ajax({
                type: "POST",
                url : "{{route('warehouse-out-item-stock-detail')}}",
                dataType: "html",
                data: {
	...
	function processAddArrayWarehouseOutRequisitionItem(){
        ...
        $.ajax({
        type: "POST",
        url : "{{route('warehouse-out-requisition-add-array')}}",
        data: {
            'item_stock_id'		: item_stock_id,
            'quantity' 	        : quantity, 
...
<nav aria-label="breadcrumb">
    <ol class="breadcrumb">
        ...
        <li class="breadcrumb-item active" aria-current="page">Tambah Permintaan Pengeluaran Gudang</li>
    </ol>
</nav>
...
<h3 class="page-title">
    <b>Form Tambah Permintaan Pengeluaran Gudang</b>
</h3>
<br/>
@if(session('msg'))
...
@endif
    ...
    <form method="post" action="{{route('process-add-warehouse-out-requisition')}}" enctype="multipart/form-data">
        @csrf
        <div class="card-body">
            <div class="row form-group">
                <div class="col-md-6">
                    <div class="form-group">
                        ...
                        {!! Form::select('warehouse_id',  $invwarehouse, $warehouseoutelements == null ? '' : $warehouseoutelements['warehouse_id'], ['class' => 'selection-search-clear select-form', 'id' => 'warehouse_id', 'onchange' => 'elements_add(this.name, this.value)']) !!}
                    </div>
                </div>
                <div class="col-md-6">
                    <div class="form-group">
                        <a class="text-dark">Tipe Pengeluaran Gudang<a class='red'> *</a></a>
                        {!! Form::select('warehouse_out_type_id',  $invwarehouseouttype, $warehouseoutelements == null ? '' : $warehouseoutelements['warehouse_out_type_id'], ['class' => 'selection-search-clear select-form', 'id' => 'warehouse_out_type_id', 'onchange' => 'elements_add(this.name, this.value)']) !!}
                    </div>
                </div>
            </div>
            <div class="row form-group">
                <div class="col-md-6">
                    <div class="form-group form-md-line-input">
                        ...
                        <input type ="date" class="form-control form-control-inline input-medium date-picker input-date" data-date-format="dd-mm-yyyy" type="text" name="warehouse_out_requisition_date" id="date" onChange="elements_add(this.name, this.value);" value="{{$warehouseoutelements == null ? '' : $warehouseoutelements['warehouse_out_requisition_date']}}" style="width: 15rem;"/>
                    </div>
                </div>
            </div>
            <div class="row form-group">
                <div class="col-md-12">
                    <div class="form-group">
                        ...
                        <textarea rows="3" type="text" class="form-control input-bb" name="warehouse_out_remark" onChange="elements_add(this.name, this.value);" id="warehouse_out_remark" >{{$warehouseoutelements == null ? '' : $warehouseoutelements['warehouse_out_remark']}}</textarea>
                    </div>
                </div>
            </div>
            <div class="row">
        ...
        <div class="card-footer text-muted">
            <div class="form-actions float-right">
                <a type="button" name="Save" class="btn btn-primary btn-sm" title="Save" onClick='processAddArrayWarehouseOutRequisitionItem()'>Tambah</a>
            </div>
        </div>
    </div>
    </div>
    ...
    <div class="card border border-dark">
                                        ...
                                        echo"
                                            <tr>
                                                ...
                                                <td style='text-align  : center'>
                                                    <a href="{{route('warehouse-out-requisition-delete-array', ['record_id' => $key])}}" name='Reset' class='btn btn-danger btn-sm' onClick='javascript:return confirm(\"apakah yakin ingin dihapus ?\")'></i> Hapus</a>
                                                </td>
                                                <?php
                                                echo"
                                            </tr>
        ...
        <div class="card-footer text-muted">
            <div class="form-actions float-right">
                <a href="{{route('reset-array-warehouse-out-requisition')}}" name='Reset' class='btn btn-danger btn-sm' onClick='javascript:return confirm(\"apakah yakin ingin dihapus ?\")'><i class="fa fa-times"></i> Reset</a>
