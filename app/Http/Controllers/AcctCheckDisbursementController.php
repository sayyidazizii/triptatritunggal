...
use App\Models\AcctCheckDisbursement;
use App\Models\AcctCheckDisbursementItem;
...
class AcctCheckDisbursementController extends Controller
{
    /**
    ...
    public function index()
    {
        ...
        $acctdisbursement    = AcctCheckDisbursement::select('acct_check_disbursement.*')
        ->where('acct_check_disbursement.data_state','=',0)
        ->where('acct_check_disbursement.check_disbursement_date','>=',$start_date)
        ->where('acct_check_disbursement.check_disbursement_date','<=',$end_date)
        ->orderBy('acct_check_disbursement.check_disbursement_date', 'DESC')
        ->get();

        return view('content/AcctCheckDisbursement/ListAcctCheckDisbursement', compact('acctdisbursement','start_date','end_date'));
    }

    public function addAcctCheckDisbursement()
    {
        ...
        return view('content/AcctCheckDisbursement/FormAddAcctCheckDisbursement', compact('corecustomer', 'salescustomer', 'acctaccount', 'acctaccountcashbank', 'acctdisbursementitem', 'acctdisbursementelements'));
    }
    ...
    public function elements_add(Request $request){
        ...
        if(!$acctdisbursementelements || $acctdisbursementelements == ''){
            ...
            $acctdisbursementelements['check_number']     = '';
            $acctdisbursementelements['check_disbursement_due_date']     = '';
    ...
    public function detailAcctCheckDisbursement($check_disbursement_id)
    {
        $acctdisbursementdetail = AcctCheckDisbursement::select('acct_check_disbursement.*', 'acct_account.account_code', 'acct_account.account_name')
        ->where('check_disbursement_id', '=', $check_disbursement_id)
        ->join('acct_account', 'acct_check_disbursement.account_id', '=', 'acct_account.account_id')
        ->first();

        $acctdisbursementitem = AcctCheckDisbursementItem::select('acct_check_disbursement_item.check_disbursement_item_id', 'acct_check_disbursement_item.check_disbursement_item_title', 'acct_check_disbursement_item.check_disbursement_item_amount','acct_account.account_code', 'acct_account.account_name')
        ->join('acct_account', 'acct_check_disbursement_item.account_id', '=', 'acct_account.account_id')
        ->where('acct_check_disbursement_item.check_disbursement_id', '=', $check_disbursement_id)
        ->get();
        ...
        $check_account_id 	= $preference_company['check_account_id'];
        $bank_account_id	= $preference_company['bank_account_id'];
        $cash_account_id	= $preference_company['cash_account_id'];

        $acctaccountcashbank	= AcctAccount::select('account_id', DB::raw('CONCAT(account_code, " - ", account_name) AS account_code'))
        ->where('data_state', '=', 0)
        ->where('parent_account_id', '=', $check_account_id)
        ->orWhere('parent_account_id', '=', $bank_account_id)
        ->get()
        ->pluck('account_code', 'account_id');

        return view('content/AcctCheckDisbursement/FormDetailAcctCheckDisbursement', compact('salescustomer', 'acctaccount', 'acctaccountcashbank', 'acctdisbursementitem', 'acctdisbursementdetail', 'check_disbursement_id'));
    }

    public function voidAcctDisbursement($check_disbursement_id)
    {
        $branch_id = User::select('branch_id')->where('user_id','=',Auth::id())->first();
        
        $acctdisbursementdetail = AcctCheckDisbursement::select('acct_disbursement.*', 'acct_account.account_code', 'acct_account.account_name')
        ->where('disbursement_id', '=', $check_disbursement_id)
        ->join('acct_account', 'acct_disbursement.account_id', '=', 'acct_account.account_id')
        ->first();

        $acctdisbursementitem = AcctCheckDisbursementItem::select('acct_disbursement_item.disbursement_item_id', 'acct_disbursement_item.disbursement_item_title', 'acct_disbursement_item.disbursement_item_amount','acct_account.account_code', 'acct_account.account_name')
        ->join('acct_account', 'acct_disbursement_item.account_id', '=', 'acct_account.account_id')
        ->where('acct_disbursement_item.disbursement_id', '=', $check_disbursement_id)
        ->get();
        ...
        return view('content/AcctDisbursement/FormVoidAcctDisbursement', compact( 'salescustomer', 'acctaccount', 'acctaccountcashbank', 'acctdisbursementitem', 'acctdisbursementdetail', 'check_disbursement_id'));
    }

    public function filterAcctCheckDisbursement(Request $request){
        ...
        return redirect('/check-disbursement');
    }
    ...
    public function selectProjectAcctDisbursement(Request $request)
    {
        ...
        return redirect('/check-disbursement/add');
    }
    ...
    public function addArrayAcctCheckDisbursementItem(Request $request)
    {
        $dataacctdisbursementitem = array(
            'record_id'				=> date('YmdHis'),
            'account_id_item'		=> $request->account_id_item,
            'check_disbursement_item_amount'	=> $request->check_disbursement_item_amount,
            'check_disbursement_item_title'	=> $request->check_disbursement_item_title,
        );
    ...
    public function deleteArrayAcctCheckDisbursementItem($record_id)
    {
        $arrayBaru			= array();
    ...
    public function processAddAcctCheckDisbursement(Request $request)
    {
        ...
        $data_AcctDisbursement = array(
            ...
            'check_disbursement_date'				=> $request->check_disbursement_date,
            'check_disbursement_due_date'				=> $request->check_disbursement_due_date,
            'check_number'				=> $request->check_number,
            'check_disbursement_title'			=> $request->check_disbursement_title,
            'account_id'					=> $request->account_id,
            'customer_id'					=> $request->customer_id,
            'check_disbursement_description'		=> $request->check_disbursement_description,
            'check_disbursement_amount_total' 	=> $request->check_disbursement_amount_total,
            'check_disbursement_token' 			=> md5(rand()),
            'data_state' 					=> 0,
            'created_id' 					=> Auth::id(),
            'created_on' 					=> date('Y-m-d H:i:s'),
        );

    //    dd($data_AcctDisbursement ); 
        ...
        $disbursement_token 				= AcctCheckDisbursement::select('check_disbursement_token')
        ->where('check_disbursement_token', '=', $data_AcctDisbursement['check_disbursement_token'])
        ->get();
        
        if (!empty($session_AcctDisbursementitem)){
            if(count($disbursement_token) == 0){
                if(AcctCheckDisbursement::create($data_AcctDisbursement)){
                    $acctdisbursement_last 		= AcctCheckDisbursement::select('check_disbursement_id', 'check_disbursement_no')
                    ->where('created_id', '=', $data_AcctDisbursement['created_id'])
                    ->orderBy('check_disbursement_id', 'DESC')
                    ->first();
                    
                    $journal_voucher_period 	= date("Ym", strtotime($data_AcctDisbursement['check_disbursement_date']));

                    $data_journal = array(
                        'branch_id'						=> $branch_id['branch_id'],
                        'journal_voucher_period' 		=> $journal_voucher_period,
                        'journal_voucher_date'			=> $data_AcctDisbursement['check_disbursement_date'],
                        'journal_voucher_title'			=> $data_AcctDisbursement['check_disbursement_title'],
                        'journal_voucher_no'			=> $acctdisbursement_last['check_disbursement_no'],
                        'journal_voucher_description'	=> $acctdisbursement_last['check_disbursement_no'],
                        'transaction_module_id'			=> $transaction_module_id['transaction_module_id'],
                        'transaction_module_code'		=> $transaction_module_code,
                        'transaction_journal_id' 		=> $acctdisbursement_last['check_disbursement_id'],
                        'transaction_journal_no' 		=> $acctdisbursement_last['check_disbursement_no'],
                        'journal_voucher_token'			=> $data_AcctDisbursement['check_disbursement_token'],
                    ...
                    $disbursement_id 		= AcctCheckDisbursement::select('check_disbursement_id')
                    ->where('created_id', $data_AcctDisbursement['created_id'])
                    ->orderBy('check_disbursement_id', 'DESC')
                    ->first();
                    ...
                    foreach($session_AcctDisbursementitem as $key=>$val){
                        $data_AcctDisbursementitem = array(
                            'check_disbursement_id'			=> $disbursement_id['check_disbursement_id'],
                            'account_id'				=> $val['account_id_item'],
                            'check_disbursement_item_title'	=> $val['check_disbursement_item_title'],
                            'check_disbursement_item_amount'	=> $val['check_disbursement_item_amount'],
                            'check_disbursement_item_token' 	=> $data_AcctDisbursement['check_disbursement_token'].$val['account_id_item'].$no
                        );

                        $no++;

                        if(AcctCheckDisbursementItem::create($data_AcctDisbursementitem)){
                            ...
                            $data_debit = array (
                                ...
                                'journal_voucher_description'	=> $data_AcctDisbursementitem['check_disbursement_item_title'],
                                'journal_voucher_amount'		=> ABS($data_AcctDisbursementitem['check_disbursement_item_amount']),
                                'journal_voucher_debit_amount'	=> ABS($data_AcctDisbursementitem['check_disbursement_item_amount']),
                                'account_id_default_status'		=> $account_id_default_status['account_default_status'],
                                'account_id_status'				=> 1,
                                'journal_voucher_item_token'	=> $data_AcctDisbursement['check_disbursement_token'].$data_AcctDisbursementitem['account_id'].$no
                            );
                            ...
                            Session::forget('dataacctdisbursementitem');
                            return redirect('/check-disbursement/add')->with('msg',$msg);
                            break;
                        }
                    }
                    ...
                    $data_credit = array (
                        ...
                        'journal_voucher_amount'		=> ABS($data_AcctDisbursement['check_disbursement_amount_total']),
                        'journal_voucher_credit_amount'	=> ABS($data_AcctDisbursement['check_disbursement_amount_total']),
                        'account_id_default_status'		=> $account_id_default_status['account_default_status'],
                        'account_id_status'				=> 0,
                        'journal_voucher_item_token'	=> $data_AcctDisbursement['check_disbursement_token'].$data_AcctDisbursementitem['account_id']."0"
                    );
                    ...
                    Session::forget('dataacctdisbursementitem');
                    return redirect('/check-disbursement')->with('msg',$msg);
                }else{
                    $msg = "Tambah Pengeluaran Kas Gagal";
                    Session::forget('dataacctdisbursementitem');
                    return redirect('/check-disbursement/add')->with('msg',$msg);
                ...
                $disbursement_id 		= AcctCheckDisbursement::select('disbursement_id')
                ->where('created_id', $data_AcctDisbursement['created_id'])
                ...
                foreach($session_AcctDisbursementitem as $key=>$val){
                    ...
                    $disbursement_item_token 	= AcctCheckDisbursementItem::select('check_disbursement_item_token')
                    ->where('check_disbursement_item_token', '=', $data_AcctDisbursementitem['check_disbursement_item_token'])
                    ->get();
    ...
    public function processVoidAcctDisbursement(Request $request){
        $disbursement_no	= $request->disbursement_no;
        
        $data = array (
            "disbursement_id"			=> $request->disbursement_id,
            "disbursement_token_void"	=> md5(rand()),
            "voided_id"					=> Auth::id(),
            "voided_on"					=> date('Y-m-d H:i:s'),
            "voided_remark" 			=> $request->voided_remark,
            'data_state'				=> 2,
        );
        $data_edit                              = AcctCheckDisbursement::findOrFail($data['disbursement_id']);
        $data_edit->disbursement_id             = $data['disbursement_id'];
        $data_edit->disbursement_token_void     = $data['disbursement_token_void'];
        ...
        $disbursement_token_void = AcctCheckDisbursement::select('disbursement_token_void')
        ->where('disbursement_token_void', $data['disbursement_token_void'])
        ->get();

        if(count($disbursement_token_void) == 0){
            if($data_edit->save()){
                $journal_voucher_id 	= AcctJournalVoucher::select('journal_voucher_id')
                ->where('transaction_journal_no', $disbursement_no)
                ->first();
                ...
                $data_journal = array (
                    "journal_voucher_id"			=> $journal_voucher_id['journal_voucher_id'],
                    "journal_voucher_token_void"	=> $data['disbursement_token_void'],
                    "voided"						=> 1,
                    "voided_id"						=> Auth::id(),
                    "voided_on"						=> date('Y-m-d H:i:s'),
                ...
                if ($data_journal_edit->save()){
                    foreach ($acctjournalvoucheritem as $keyItem => $valItem) {
                        $data_journal_item = array (
                            ...
                            "journal_voucher_item_token_void"	=> $data['disbursement_token_void'].$valItem['journal_voucher_item_id'],
