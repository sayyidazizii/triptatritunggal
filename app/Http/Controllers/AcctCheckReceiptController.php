...
use App\Models\AcctCheckReceipt;
use App\Models\AcctCheckReceiptItem;
...
class AcctCheckReceiptController extends Controller
{
    /**
    ...
    public function index()
    {
        ...
        $acctreceipt    = AcctCheckReceipt::select('acct_check_receipt.*')
        ->where('acct_check_receipt.data_state','=',0)
        ->where('acct_check_receipt.check_receipt_date','>=',$start_date)
        ->where('acct_check_receipt.check_receipt_date','<=',$end_date)
        ->orderBy('acct_check_receipt.check_receipt_id', 'DESC')
        ->get();

        return view('content/AcctCheckReceipt/ListAcctCheckReceipt', compact('acctreceipt','start_date','end_date'));
    }
    ...
    //     return redirect('/check-receipt/add');
    // }

    public function addAcctCheckReceipt()
    {
        ...
        ->orWhere('parent_account_id', '=', $check_account_id)
        ->get()
        ->pluck('account_code', 'account_id');
        ...
        return view('content/AcctCheckReceipt/FormAddAcctCheckReceipt', compact('salescustomer', 'acctaccount', 'acctaccountcashbank', 'acctreceiptitem', 'acctreceiptelements', 'corecustomer'));
    }
    ...
    public function elements_add(Request $request){
        ...
        if(!$acctreceiptelements || $acctreceiptelements == ''){
            ...
            $acctreceiptelements['check_receipt_date']     = '';
            $acctreceiptelements['check_number']     = '';
            $acctreceiptelements['check_receipt_due_date']     = '';
            $acctreceiptelements['account_id']           = '';   
            $acctreceiptelements['customert_id']           = '';   
            $acctreceiptelements['check_receipt_title']  = '';
            $acctreceiptelements['check_receipt_description']   = '';
        }
        $acctreceiptelements[$request->name] = $request->value;
        Session::put('acctreceiptelements', $acctreceiptelements);
    }

    public function detailAcctCheckReceipt($check_receipt_id)
    {
        $acctreceiptdetail = AcctCheckReceipt::select('acct_check_receipt.*', 'acct_account.account_code', 'acct_account.account_name')
        ->where('check_receipt_id', '=', $check_receipt_id)
        ->join('acct_account', 'acct_check_receipt.account_id', '=', 'acct_account.account_id')
        ->first();

        $acctreceiptitem = AcctCheckReceiptItem::select('acct_check_receipt_item.check_receipt_item_id', 'acct_check_receipt_item.check_receipt_item_title', 'acct_check_receipt_item.check_receipt_item_amount','acct_account.account_code', 'acct_account.account_name')
        ->join('acct_account', 'acct_check_receipt_item.account_id', '=', 'acct_account.account_id')
        ->where('acct_check_receipt_item.check_receipt_id', '=', $check_receipt_id)
        ->get();
        ...
        $cash_account_id 	= $preference_company['cash_account_id'];
        $bank_account_id	= $preference_company['bank_account_id'];
        $check_account_id	= $preference_company['bank_account_id'];
        ...
        return view('content/AcctCheckReceipt/FormDetailAcctCheckReceipt', compact('salescustomer', 'acctaccount', 'acctaccountcashbank', 'acctreceiptitem', 'acctreceiptdetail'));
    }
    
    public function voidAcctCashReceipt($cash_receipt_id)
    {
        $acctreceiptdetail = AcctCheckReceipt::select('acct_cash_receipt.*', 'acct_account.account_code', 'acct_account.account_name')
        ->where('cash_receipt_id', '=', $cash_receipt_id)
        ->join('acct_account', 'acct_cash_receipt.account_id', '=', 'acct_account.account_id')
        ->first();

        $acctreceiptitem = AcctCheckReceiptItem::select('acct_cash_receipt_item.cash_receipt_item_id', 'acct_cash_receipt_item.cash_receipt_item_title', 'acct_cash_receipt_item.cash_receipt_item_amount','acct_account.account_code', 'acct_account.account_name')
    ...
    public function processVoidAcctCashReceipt(Request $request)
    {
        ...
        $data_edit                      = AcctCheckReceipt::findOrFail($data['cash_receipt_id']);
        ...
        $receipt_token_void = AcctCheckReceipt::select('cash_receipt_token_void')
        ...
        if(count($receipt_token_void) == 0){
            if($data_edit->save()){
                ...
                $msg = "Pembatalan Penerimaan Giro Sukses";
                redirect('/check-receipt')->with('msg',$msg);
            }else{
                $msg = "Pembatalan Penerimaan Giro Gagal";
                redirect('/check-receipt')->with('msg',$msg);
            }
        } else {
            $journal_voucher_id 	= AcctJournalVoucher::select('journal_voucher_id')
            ...
            $msg = "Pembatalan Penerimaan Giro Sukses";
            return redirect('/check-receipt')->with('msg',$msg);
        }
            
    }

    public function filterAcctCheckReceipt(Request $request){
    ...
    public function addArrayAcctCheckReceiptItem(Request $request)
    {
        $dataacctreceiptitem = array(
            'record_id'				    => date('YmdHis'),
            'account_id_item'		    => $request->account_id_item,
            // 'customer_id'		        => $request->customer_id,
            'check_receipt_item_amount'	=> $request->check_receipt_item_amount,
            'check_receipt_item_title'	=> $request->check_receipt_item_title,
        );
    ...
    public function deleteArrayAcctCheckReceiptItem($record_id)
    {
        $arrayBaru			= array();
    ...
    public function processAddAcctCheckReceipt(Request $request)
    {
        ...
        $data_AcctReceipt = array(
            ...
            'check_receipt_date'		    => $request->check_receipt_date,
            'account_id'				    => $request->account_id,
            'check_receipt_due_date'		    => $request->check_receipt_due_date,
            'check_number'				    => $request->check_number,
            'customer_id'				    => $request->customer_id,
            'check_receipt_title'			=> $request->check_receipt_title,
            'check_receipt_description'		=> $request->check_receipt_description,
            'check_receipt_amount_total' 	=> $request->check_receipt_amount_total,
            'check_receipt_token' 			=> md5(rand()),
            'data_state' 				    => 0,
            'created_id' 				    => Auth::id(),
        ...
        $receipt_token 				= AcctCheckReceipt::select('check_receipt_token')
        ->where('check_receipt_token', '=', $data_AcctReceipt['check_receipt_token'])
        ->get();
        
        if (!empty($session_AcctReceiptitem)){
            if(count($receipt_token) == 0){
                if(AcctCheckReceipt::create($data_AcctReceipt)){
                    $acctreceipt_last 		= AcctCheckReceipt::select('check_receipt_id', 'check_receipt_no')
                    ->where('created_id', '=', $data_AcctReceipt['created_id'])
                    ->orderBy('check_receipt_id', 'DESC')
                    ->first();
                    
                    $journal_voucher_period 	= date("Ym", strtotime($data_AcctReceipt['check_receipt_date']));

                    $data_journal = array(
                        'branch_id'						=> $branch_id['branch_id'],
                        'journal_voucher_period' 		=> $journal_voucher_period,
                        'journal_voucher_date'			=> $data_AcctReceipt['check_receipt_date'],
                        'journal_voucher_title'			=> $data_AcctReceipt['check_receipt_title'],
                        'journal_voucher_no'			=> $acctreceipt_last['check_receipt_no'],
                        'journal_voucher_description'	=> $data_AcctReceipt['check_receipt_description'],
                        'transaction_module_id'			=> $transaction_module_id['transaction_module_id'],
                        'transaction_module_code'		=> $transaction_module_code,
                        'transaction_journal_id' 		=> $acctreceipt_last['check_receipt_id'],
                        'transaction_journal_no' 		=> $acctreceipt_last['check_receipt_no'],
                        'created_id' 					=> $data_AcctReceipt['created_id'],
                        'journal_voucher_token' 		=> $data_AcctReceipt['check_receipt_token'],
                    ...
                    $data_debit = array (
                        ...
                        'journal_voucher_amount'		=> ABS($data_AcctReceipt['check_receipt_amount_total']),
                        'journal_voucher_debit_amount'	=> ABS($data_AcctReceipt['check_receipt_amount_total']),
                        'account_id_default_status'		=> $account_id_default_status['account_default_status'],
                        'account_id_status'				=> 1,
                        'journal_voucher_item_token'	=> $data_AcctReceipt['check_receipt_token'].$data_AcctReceipt['account_id'].'1'
                    );

                    AcctJournalVoucherItem::create($data_debit);

                    $check_receipt_id = AcctCheckReceipt::select('check_receipt_id')
                    ->where('created_id', '=', $data_AcctReceipt['created_id'])
                    ->orderBy('check_receipt_id', 'DESC')
                    ->first();
                    ...
                    foreach($session_AcctReceiptitem as $key=>$val){
                        $data_AcctReceiptitem = array(
                            'check_receipt_id'			=> $check_receipt_id['check_receipt_id'],
                            'account_id'			    => $val['account_id_item'],
                            'check_receipt_item_title'	=> $val['check_receipt_item_title'],
                            'check_receipt_item_amount'	=> $val['check_receipt_item_amount'],
                            'check_receipt_item_token'	=> md5(rand()).$val['account_id_item']
                        );


                        if(AcctCheckReceiptItem::create($data_AcctReceiptitem)){	
                            $account_id_default_status 	= AcctAccount::select('account_default_status')
                            ...
                            $data_credit = array (
                                ...
                                'journal_voucher_description'	=> $data_AcctReceiptitem['check_receipt_item_title'],
                                'journal_voucher_amount'		=> ABS($data_AcctReceiptitem['check_receipt_item_amount']),
                                'journal_voucher_credit_amount'	=> ABS($data_AcctReceiptitem['check_receipt_item_amount']),
                                'account_id_default_status'		=> $account_id_default_status['account_default_status'],
                                'account_id_status'				=> 0,
                            ...
                            $msg = "Tambah Data Penerimaan Giro Tidak Berhasil";
                            Session::forget('dataacctreceiptitem');
                            return redirect('/check-receipt/add')->with('msg',$msg);
                            break;
                        }
                    }
                    Session::forget('dataacctreceiptitem');

                    $msg = "Tambah Data Penerimaan Giro Berhasil";
                    return redirect('/check-receipt')->with('msg',$msg);
                }else{
                    $msg = "Tambah Data Penerimaan Giro Tidak Berhasil";
                    Session::forget('dataacctreceiptitem');
                ...
                foreach($session_AcctReceiptitem as $key=>$val){
                    $data_AcctReceiptitem = array(
                        'check_receipt_id'			=> $check_receipt_id['receipt_id'],
                        'account_id'			=> $val['account_id_item'],
                        'check_receipt_item_title'	=> $val['check_receipt_item_title'],
                        'check_receipt_item_amount'	=> $val['check_receipt_item_amount'],
                        'check_receipt_item_token'	=> md5(rand()).$val['account_id_item']
                    ...
                    $receipt_item_token = AcctCheckReceiptItem::select('check_receipt_item_token')
                    ->where('check_receipt_item_token', '=', $data_AcctReceiptitem['check_receipt_item_token'])
                    ->get();
            ...
            $msg = "Data Penerimaan Giro Kosong";
            Session::forget('dataacctreceiptitem');
            return redirect('/check-receipt/add')->with('msg',$msg);
        }
    }
