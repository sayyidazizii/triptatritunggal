...
class AcctJournalVoucherSalesController extends Controller
{
    /**
    ...
    public function index()
    {
        ...
        ->where('acct_journal_voucher.data_state','=',0)
         ->where('acct_journal_voucher.transaction_module_code','=','SI')
       ...
       // dd($acctjournalvoucher);
        return view('content/AcctJournalVoucher/ListAcctJournalVoucherSales', compact('acctjournalvoucher','start_date','end_date','corebranch','branch_id'));
    }
    ...
    public function processPrintingAcctJournalVoucher ()
    {
        ...
        <table cellspacing=\"0\" cellpadding=\"2\" border=\"0\">
            <tr>
                <td><div style=\"text-align: center; font-size:14px; font-weight: bold\">JURNAL PENJUALAN</div></td>
            </tr>
            <tr>
        ...
        $filename = 'Jurnal_Penjualan_'.$start_date.'s.d.'.$end_date.'.pdf';
    ...
    public function processExportAcctJournalVoucher()
    {
        ...
        if(count($acctjournalvoucher)>=0){
            $spreadsheet->getProperties()->setCreator("KAROTA KING")
                                         ...
                                         ->setSubject("")
                                         ->setDescription("Jurnal Penjualan")
                                         ->setKeywords("Jurnal Penjualan")
                                         ->setCategory("Jurnal Penjualan");
                                 
            $sheet = $spreadsheet->getActiveSheet(0);
            $spreadsheet->getActiveSheet()->setTitle("Jurnal Penjualan");
            ...
            $sheet->setCellValue('B1',"Jurnal Penjualan Dari Periode ".date('d M Y', strtotime($start_date))." s.d. ".date('d M Y', strtotime($end_date)));	
            ...
            // ob_clean();
            $filename='Jurnal_penjualan_'.$start_date.'_s.d._'.$end_date.'.xls';
