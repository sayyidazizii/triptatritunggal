...
class GradingController extends Controller
{
    ...
    public function addGrading($item_stock_id)
    {
        ...
        $package = InvItemStock::where('data_state', 0)
        ->where('item_category_id', 3)
    ...
    public function processAddGrading(Request $request)
    {
        ...
        foreach($dataarrayinvitemstock as $item){
            ...
            if($invitemstockmaterial->save()){
                $invitemmaterial = InvItem::where('item_id', $invitem_material['item_id'])
                ->first();

                $invitemmaterial->hpp_amount = ( ($total_start * $hpp_start) - ($total_add * $hpp_add) ) / ($total_start - $total_add);
                $invitemmaterial->save();
                ...
                foreach($item['package'] as $package){
                    ...
                    if($invitemstock->save()){
                        ...
                        $hpp_package = $hpp_package + ($purchaseorderitempackage['item_unit_cost'] * $total_package);
                    }else{
            ...
            if(InvItemStock::create($itemstock)){
                ...
                $invitemstock = InvItemStock::select('inv_item_stock.*', 'inv_item_unit.item_unit_default_quantity')
                ->where('item_id', $invitem_production['item_id'])
                ->join('inv_item_unit', 'inv_item_unit.item_unit_id', 'inv_item_stock.item_unit_id')
                ->where('inv_item_stock.data_state', 0)
                ->get();
...
//----------------------------------------------------------Journal Voucher-------------------------------------------------------------------//
            ...
            // if($purchaseorder['purchase_order_type_id'] == 2){
                ...
                $account 		= AcctAccount::where('account_id', $invitemstockmaterial['inv_account_id'])
                ->where('data_state', 0)
                ->first();
                ...
                $data_credit = array (
                    ...
                    'account_id'					=> $invitemstockmaterial['inv_account_id'],
                ...
                foreach($item['package'] as $package){
                    ...
                    $invitemstockpackage = InvItemStock::where('item_stock_id', $package['package_id'])->first();

                    $account 		= AcctAccount::where('account_id', $invitemstockpackage['inv_account_id'])
                    ->where('data_state', 0)
                    ->first();
                    ...
                    $data_credit = array (
                        'journal_voucher_id'			=> $journal_voucher_id,
                        'account_id'					=> $invitemstockpackage['inv_account_id'],
                        'journal_voucher_description'	=> $data_journal['journal_voucher_description'],
                ...
                $account 		= AcctAccount::where('account_id', $invitemstocklast['inv_account_id'])
                ->where('data_state', 0)
                ->first();
                ...
                $data_debit = array (
                    'journal_voucher_id'			=> $journal_voucher_id,
                    'account_id'					=> $invitemstocklast['inv_account_id'],
                    'journal_voucher_description'	=> $data_journal['journal_voucher_description'],
...
//--------------------------------------------------------End Journal Voucher-----------------------------------------------------------------//
    ...
    public function processEditGrading(Request $request)
    {
        ...
        $packageold = InvItemStockPackage::where('item_stock_id', $fields['item_stock_id'])
        ->get();

        foreach($packageold as $val){
            //tempat simpan jurnal buat loss/ kerugian packaging lama
        }
        ...
        foreach($dataarrayinvitemstock as $item){
            ...
            $item_total = $invitemstockmaterial['item_total'] - ($item['item_quantity'] / $itemunitfirst['item_unit_default_quantity'] * $itemunitsecond['item_unit_default_quantity']);
            ...
            if($invitemstockmaterial->save()){
                foreach($item['package'] as $package){
                    ...
                    $item_total = $invitemstock['item_total'] - ($package['item_package_quantity'] / $itemunitfirst['item_unit_default_quantity'] * $itemunitsecond['item_unit_default_quantity']);
