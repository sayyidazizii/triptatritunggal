...
class AcctJournalVoucherPurchaseController extends Controller
{
    /**
    ...
    public function index()
    {
        ...
        ->where('acct_journal_voucher.data_state','=',0)
         ->where('acct_journal_voucher.transaction_module_code','=','PI')
       ...
       // dd($acctjournalvoucher);
        return view('content/AcctJournalVoucher/ListAcctJournalVoucherPurchase', compact('acctjournalvoucher','start_date','end_date','corebranch','branch_id'));
    }
    ...
    public function filterAcctJournalVoucher(Request $request){
        ...
        return redirect('/journal-Purchase');
    }
    ...
    public function processPrintingAcctJournalVoucher ()
    {
        ...
        <table cellspacing=\"0\" cellpadding=\"2\" border=\"0\">
            <tr>
                <td><div style=\"text-align: center; font-size:14px; font-weight: bold\">JURNAL PEMBELIAN</div></td>
            </tr>
            <tr>
        ...
        $filename = 'Jurnal_Umum_'.$start_date.'s.d.'.$end_date.'.pdf';
    ...
    public function processExportAcctJournalVoucher()
    {
        ...
        if(count($acctjournalvoucher)>=0){
            $spreadsheet->getProperties()->setCreator("KAROTA KING")
                                         ...
                                         ->setSubject("")
                                         ->setDescription("Jurnal Pembelian")
                                         ->setKeywords("Jurnal Pembelian")
                                         ->setCategory("Jurnal Pembelian");
                                 
            $sheet = $spreadsheet->getActiveSheet(0);
            $spreadsheet->getActiveSheet()->setTitle("Jurnal Pembelian");
            ...
            $sheet->setCellValue('B1',"Jurnal Pembelian Dari Periode ".date('d M Y', strtotime($start_date))." s.d. ".date('d M Y', strtotime($end_date)));	
            ...
            // ob_clean();
            $filename='Jurnal_Pembelian_'.$start_date.'_s.d._'.$end_date.'.xls';
